...
def get_dataloader():
    ...
    diagnosis_data = np.fill_diagonal(np.load(Path(path) / 'diagnoses_scores_1000.npy').astype('float16'), -np.inf)
    age_data = np.fill_diagonal(np.load(Path(path) / 'age_scores_1000.npy').astype('float16'), -np.inf)
    gender_data = np.fill_diagonal(np.load(Path(path) / 'gender_scores_1000.npy').astype('float16'), -np.inf)
